...
class RepositorioTrabajadores:
    ...
    def listar_por_area(self, area: str) -> List[Trabajador]:
        return [trabajador for trabajador in self._trabajadores if trabajador.get_area() == area]
    ...
    def validar_reglas_de_negocio(self) -> list[dict]:
        ...
        for area, total in asistentes_por_area.items():
            validaciones.append(
                {
                    "cumple": total <= 2,
                    "mensaje": f"Área {area}: asistentes registrados = {total} (máximo permitido: 2).",
                }
            )
        ...
        for area, total in tecnicos_por_area.items():
            validaciones.append(
                {
                    "cumple": total <= 5,
                    "mensaje": f"Área {area}: técnicos registrados = {total} (máximo permitido: 5).",
                }
            )
